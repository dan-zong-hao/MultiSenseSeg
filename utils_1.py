...
def get_dataset_conf(dataset_name):
    ...
    if dataset_name == 'Vaihingen':
        conf['ids'] = ['1', '3', '23', '26', '7', '11', '13', '28', '17', '32', '34', '37']
        conf['test_ids'] = ['5', '21', '15', '30']
        conf['stride'] = 32
        conf['folder'] = FOLDER + 'Vaihingen/'
        conf['img_path'] = conf['folder'] + 'top/top_mosaic_09cm_area{}.tif'
        conf['dsm_path'] = conf['folder'] + 'dsm/dsm_09cm_matching_area{}.tif'
        conf['lbl_path'] = conf['folder'] + 'gts_for_participants/top_mosaic_09cm_area{}.tif'
        conf['ero_path'] = conf['folder'] + 'gts_eroded_for_participants/top_mosaic_09cm_area{}_noBoundary.tif'
        conf['label'] = ["roads", "buildings", "low veg.", "trees", "cars", "clutter"] # Label names
        conf['palette'] = {
            ...
            6 : (0, 0, 0)}       # built-up area
        
    elif dataset_name == 'Potsdam':
        conf['ids'] = ['6_10', '7_10', '2_12', '3_11', '2_10', '7_8', '5_10', '3_12', '5_12', '7_11', '7_9', '6_9', '7_7', '4_12', '6_8', '6_12', '6_7', '4_11']
        conf['test_ids'] = ['4_10', '5_11', '2_11', '3_10', '6_11', '7_12']
        conf['stride'] = 128
        conf['folder'] = FOLDER + 'Potsdam/'
        conf['img_path'] = conf['folder'] + '4_Ortho_RGBIR/top_potsdam_{}_RGBIR.tif'
        conf['dsm_path'] = conf['folder'] + '1_DSM_normalisation/dsm_potsdam_{}_normalized_lastools.jpg'
        conf['lbl_path'] = conf['folder'] + '5_Labels_for_participants/top_potsdam_{}_label.tif'
        conf['ero_path'] = conf['folder'] + '5_Labels_for_participants_no_Boundary/top_potsdam_{}_label_noBoundary.tif'
        conf['label'] = ["roads", "buildings", "low veg.", "trees", "cars", "clutter"] # Label names
        conf['palette'] = {
            0 : (255, 255, 255), # Impervious surfaces (white)
            1 : (0, 0, 255),     # Buildings (blue)
...
# 加载配置
train_conf = get_dataset_conf(DATASET)
test_conf = get_dataset_conf(TEST_DATASET)
train_ids = train_conf['ids']
palette = train_conf['palette']
invert_palette = {v: k for k, v in palette.items()}
LABELS = train_conf['label']
N_CLASSES = len(LABELS) # Number of classes
WEIGHTS = torch.ones(N_CLASSES) # Weights for class balancing

# 训练用的路径变量 (ISPRS_dataset 可能会用到这些，如果不修改Dataset类，需要保持变量名)
# 假设 Dataset 类直接使用 DATA_FOLDER，我们这里先赋值给训练用的
DATA_FOLDER = train_conf['img_path']
DSM_FOLDER = train_conf['dsm_path']
LABEL_FOLDER = train_conf['lbl_path']
ERODED_FOLDER = train_conf['ero_path']    

test_ids = test_conf['test_ids'] 

# [核心] 定义专门给 test() 函数用的路径
TEST_DATA_FOLDER = test_conf['img_path']
TEST_DSM_FOLDER = test_conf['dsm_path']
TEST_LABEL_FOLDER = test_conf['lbl_path']
TEST_ERODED_FOLDER = test_conf['ero_path']

# Stride 使用测试集的建议 Stride
Stride_Size = test_conf['stride']
...
def metrics(predictions, gts, label_values=LABELS, logger: logging.Logger | None = None):
    log = logger.info if logger is not None else print

    cm = confusion_matrix(gts, predictions, labels=range(len(label_values)))

    log("Confusion matrix :")
    log("\n%s", cm) if logger is not None else log(cm)

    total = cm.sum()
    accuracy = np.trace(cm) * 100.0 / float(total)
    log("%d pixels processed", int(total)) if logger is not None else log("%d pixels processed" % int(total))
    log("Total accuracy : %.2f", float(accuracy)) if logger is not None else log("Total accuracy : %.2f" % float(accuracy))

    Acc = np.diag(cm) / cm.sum(axis=1)
    for l_id, score in enumerate(Acc):
        if logger is not None:
            log("%s: %.4f", label_values[l_id], float(score))
        else:
            log("%s: %.4f" % (label_values[l_id], float(score)))
    log("---")

    F1Score = np.zeros(len(label_values), dtype=np.float64)
    for i in range(len(label_values)):
    ...
    mean_f1 = np.nanmean(F1Score[:5])
    log("mean F1Score: %.4f", float(mean_f1)) if logger is not None else log("mean F1Score: %.4f" % float(mean_f1))
    log("---")
    ...
    log("Kappa: %.4f", float(kappa)) if logger is not None else log("Kappa: %.4f" % float(kappa))

    MIoU = np.diag(cm) / (np.sum(cm, axis=1) + np.sum(cm, axis=0) - np.diag(cm))
    if logger is not None:
        log("MIoU per class: %s", np.array2string(MIoU, precision=4, suppress_small=False))
    else:
        log(MIoU)

    mean_miou = np.nanmean(MIoU[:5])
    log("mean MIoU: %.4f", float(mean_miou)) if logger is not None else log("mean MIoU: %.4f" % float(mean_miou))
    log("---")

    return mean_miou


def metrics_loveda(predictions, gts, label_values=LABELS, logger: logging.Logger | None = None):
    log = logger.info if logger is not None else print
...
def setup_logger(log_dir: str, name: str = "test_logger") -> logging.Logger:
    ...
    # 防止你在 notebook/多次运行时重复添加 handler
    if logger.handlers:
        return logger
